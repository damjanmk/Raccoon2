...
#         print self.app.engine.vina_settings
        ...
        instances = 1#science fair int( self.app.engine.guseNumberOfInstances.get() )
        print instances
...
#             msg = "Execution time: " + str(timeNow)
#             self.queue.put(msg)
#             sleep(2)
...
#        os.rename("../gUSE-cloud-vina-new.zip", "../gUSE-cloud-vina.zip")
    ...
    def process_detailsinfo(self, wfstatus):#, startTime):    
        i = 0 # counting which substring we're on - if 0, print 'submitted', or 'running' etc.
        init = 0
        running = 0
        finished = 0
        error = 0
        ...
        for wfstatusSegment in wfstatus.split(";"):        
            if i == 0:
                if wfstatusSegment == "submitted":
                    self.queue.put("Raccoon execution via gUSE submitted.")
                    returnVal = 2
                elif wfstatusSegment == "running":
                    self.queue.put("Raccoon execution via gUSE running...")
                    returnVal = 2
                elif wfstatusSegment == "finished":
                    self.queue.put("Raccoon execution via gUSE finished successfully!")
                    self.queue.put("Please open the folder \'gUSE-cloud-vina-filtered-results-\' + today\'s date (timestamp) to view the results.")
                    returnVal = 1
                    ...
                    self.queue.put("Raccoon execution via gUSE stopped by administrator")
                elif wfstatusSegment == "not valid data":
                    self.queue.put("Data for Raccoon execution via gUSE not valid")
        ...
        if init > 0:
            ...
            if init == 1:
                isare = " is"
                self.queue.put(str(init) + "/" + total + " job" + isare + " initialising...")
        if running > 0:
            isare = "s are"
            if running == 1:
                isare = " is"
            self.queue.put(str(running) + "/" + total + " job" + isare + " running...")
        if finished > 0:
            hashave = "s have"
            if finished == 1:
                hashave = " has"
        ...
        if error > 0:
            self.queue.put(str(error) + "/" + total + " jobs had some errors.")
    ...
    def checkGuseStatus(self, wfidsList, RemoteAPIPassword, gUSEurl):        
        ...
        while True:
            ...
            for wfid in wfidsList:                
                ...
                try:
                    #"submitted;job1name:running=10:finished=0;"
